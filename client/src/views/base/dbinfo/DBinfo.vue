    ...
    <b-card header-tag="header">
      <div slot="header">
        ...
        <div class="card-actions">
          <!-- 命名路由 -->
          <strong>
            <b-link :to="{ name: 'dbinfoAdd' }" :disabled="btnDisabled">添加</b-link>
          </strong>
        </div>
      </div>
      <b-table :hover="hover" :striped="striped" :bordered="bordered" :small="small" :fixed="fixed" responsive="sm" :items="items" :fields="fields" :current-page="currentPage" :per-page="perPage">
        ...
        <template slot="edit_details" slot-scope="row">
          <b-button :disabled="btnDisabled" :to="{ name: 'dbinfoEdit', params: { dId: row.item._id }}" size="sm" class="mr-2" variant="success">
            修改
          </b-button>
        </template>
        <template slot="del_details" slot-scope="row">
          <b-button :disabled="btnDisabled" @click.stop="info(row.item.Number, row.item._id, $event.target)" size="sm" class="mr-2" variant="danger">
            删除
          </b-button>
        </template>
